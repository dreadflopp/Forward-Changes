...
{
    public class TaperCurveHandler : AbstractPropertyHandler<float>
    {
        public override string PropertyName => "TaperCurve";

        public override void SetValue(IMajorRecord record, float value)
        {
            if (record is IMagicEffect magicEffect)
            {
                magicEffect.TaperCurve = value;
            }
            else
            {
