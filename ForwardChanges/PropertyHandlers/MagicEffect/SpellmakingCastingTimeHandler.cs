...
{
    public class SpellmakingCastingTimeHandler : AbstractPropertyHandler<float>
    {
        public override string PropertyName => "SpellmakingCastingTime";

        public override void SetValue(IMajorRecord record, float value)
        {
            if (record is IMagicEffect magicEffect)
            {
                magicEffect.SpellmakingCastingTime = value;
            }
            else
            {
        ...
        public override float GetValue(IMajorRecordGetter record)
        {
            ...
            {
                return magicEffect.SpellmakingCastingTime;
            }
            else
            {
