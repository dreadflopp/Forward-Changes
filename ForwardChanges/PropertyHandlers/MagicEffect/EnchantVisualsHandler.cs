...
{
    public class EnchantVisualsHandler : AbstractPropertyHandler<IFormLinkGetter<IVisualEffectGetter>>
    {
        public override string PropertyName => "EnchantVisuals";
        ...
        public override void SetValue(IMajorRecord record, IFormLinkGetter<IVisualEffectGetter>? value)
        {
            if (record is IMagicEffect magicEffect)
            {
                ...
                {
                    magicEffect.EnchantVisuals = new FormLink<IVisualEffectGetter>(value.FormKey);
                }
                else
                {
                    magicEffect.EnchantVisuals.Clear();
                }
            }
            else
            {
        ...
        public override IFormLinkGetter<IVisualEffectGetter>? GetValue(IMajorRecordGetter record)
        {
            ...
            {
                return magicEffect.EnchantVisuals;
            }
            else
            {
