...
{
    public class LocationReferenceCellStaticReferencesListPropertyHandler : AbstractListPropertyHandler<IFormLinkGetter<IPlacedSimpleGetter>>
    {
        public override string PropertyName => "ReferenceCellStaticReferences";
        ...
        public override List<IFormLinkGetter<IPlacedSimpleGetter>>? GetValue(IMajorRecordGetter record)
        {
            ...
            {
                return locationRecord.ReferenceCellStaticReferences?.ToList();
            }
        ...
        public override void SetValue(IMajorRecord record, List<IFormLinkGetter<IPlacedSimpleGetter>>? value)
        {
            ...
            {
                if (locationRecord.ReferenceCellStaticReferences != null)
                {
                    locationRecord.ReferenceCellStaticReferences.Clear();
                    if (value != null)
                    {
                        foreach (var item in value)
                        {
                            locationRecord.ReferenceCellStaticReferences.Add(item);
                        }
                    }
                }
            }
