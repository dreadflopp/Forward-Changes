...
{
    public class PlacedObjectRotationPropertyHandler : AbstractPropertyHandler<P3Float?>
    {
        public override string PropertyName => "Placement.Rotation";
        ...
        public override void SetValue(IMajorRecord record, P3Float? value)
        {
            if (record is IPlacedObject placedObject)
            {
                if (value.HasValue)
                {
                    ...
                    placedObject.Placement.Rotation = value.Value;
                }
            }
            else
            {
        ...
        public override P3Float? GetValue(IMajorRecordGetter record)
        {
            ...
            {
                return placedObject.Placement?.Rotation;
            }
            else
            {
                Console.WriteLine($"Error: Record is not a PlacedObject for {PropertyName}");
