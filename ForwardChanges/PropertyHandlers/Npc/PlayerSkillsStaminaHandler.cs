...
{
    public class PlayerSkillsStaminaHandler : AbstractPropertyHandler<ushort>
    {
        public override string PropertyName => "PlayerSkills.Stamina";

        public override ushort GetValue(IMajorRecordGetter record)
        {
            if (record is INpcGetter npcRecord)
            {
                return npcRecord.PlayerSkills?.Stamina ?? 0;
            }
            else
            {
        ...
        public override void SetValue(IMajorRecord record, ushort value)
        {
            if (record is INpc npcRecord)
            {
                ...
                npcRecord.PlayerSkills.Stamina = value;
            }
            else
            {
