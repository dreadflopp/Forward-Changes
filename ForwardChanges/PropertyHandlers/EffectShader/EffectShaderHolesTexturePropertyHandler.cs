...
{
    public class EffectShaderHolesTexturePropertyHandler : AbstractPropertyHandler<AssetLinkGetter<SkyrimTextureAssetType>>
    {
        public override string PropertyName => "HolesTexture";
        ...
        public override void SetValue(IMajorRecord record, AssetLinkGetter<SkyrimTextureAssetType>? value)
        {
            if (record is IEffectShader effectShader)
            {
                ...
                {
                    effectShader.HolesTexture = new AssetLink<SkyrimTextureAssetType>(value.ToString());
                }
                else
                {
                    effectShader.HolesTexture = null;
                }
            }
            else
            {
        ...
        public override AssetLinkGetter<SkyrimTextureAssetType>? GetValue(IMajorRecordGetter record)
        {
            ...
            {
                return effectShader.HolesTexture;
            }
            else
            {
                Console.WriteLine($"Error: Record is not an EffectShader for {PropertyName}");
